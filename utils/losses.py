...
def NPNloss(labels, logits, logits_b, coords_b, logits_b_pmx, coords_b_pmx, config):

    loss = torch.tensor(0.0, device=labels.device)

    # L_CN = l1_loss(feat1=logits_b,
    #                feat2=logits_b_pmx,
    #                coords1=coords_b,
    #                coords2=coords_b_pmx)
    L_CE = F.cross_entropy(logits, labels)

    negative_mask = torch.ones_like(logits).scatter_(1, labels.unsqueeze(1), 0)

    probs = F.softmax(logits, dim=1)
    L_NL = -torch.mean(torch.sum(torch.log(1. - probs + 1e-7) * negative_mask, dim=1))

    penalty = conf_penalty(logits)

    # loss = config.alpha * L_NL  + config.beta * L_CN + config.gamma * L_CE + penalty 
    loss = config.alpha * L_NL + config.gamma * L_CE + penalty 
    return loss

def l1_loss(feat1, feat2, coords1, coords2, conf_thresh=0.7):

    idx1, idx2 = match_coords(coords1, coords2)
    if len(idx1) == 0:
        return torch.tensor(0.0, device=feat1.device)

    idx1 = torch.tensor(idx1, dtype=torch.long, device=feat1.device)
    idx2 = torch.tensor(idx2, dtype=torch.long, device=feat1.device)

    # 获取匹配特征
    logit1_match = feat1[idx1]  # [K, C]
    logit2_match = feat2[idx2]  # [K, C]

    # 置信度筛选
    prob2 = F.softmax(logit2_match, dim=1)
    conf2, _ = prob2.max(dim=1)

    mask = conf2 > conf_thresh
    if mask.sum() == 0:
        return torch.tensor(0.0, device=feat1.device)

    # 选出高置信度的匹配点
    f1_sel = logit1_match[mask]
    f2_sel = logit2_match[mask]

    # 可选归一化
    f1_sel = F.normalize(f1_sel, dim=-1)
    f2_sel = F.normalize(f2_sel, dim=-1)

    # 手写 L1 损失：|f1 - f2|.mean()
    loss = (f1_sel - f2_sel).abs().mean()
    return loss

def match_coords(coord1, coord2):

    coord1_np = coord1.cpu().numpy()
    coord2_np = coord2.cpu().numpy()
    
    coord1_set = {tuple(pt) for pt in coord1_np}
    coord2_map = {tuple(pt): i for i, pt in enumerate(coord2_np)}

    matched_idx1, matched_idx2 = [], []
    for i, pt in enumerate(coord1_np):
        key = tuple(pt)
        if key in coord2_map:
            matched_idx1.append(i)
            matched_idx2.append(coord2_map[key])
