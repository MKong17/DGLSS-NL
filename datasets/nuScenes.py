...
class nuScenesDataset(CustomDataset):
    ...
    def __getitem__(self, index):
        ...
        start_index = lidar_path.find('samples/')
        if start_index != -1:
            lidar_path = lidar_path[start_index:]  # 只保留从 'samples/' 开始的路径
        else:
            raise ValueError(f"❌ lidar_path 格式错误，未包含 'samples/': {lidar_path}")
        lidar_sd_token = self.nusc.get('sample', info['sample_token'])['data']['LIDAR_TOP']
