...
class CustomDataset(Dataset):
    ...
    def make_positive_samples(self, xyz, ref, semantic_label):
        ...
        # print(len(return_label))
        # print(return_label[0].shape)
        for _ in range(self.positive_num):
            ...
            # print(mask.shape, semantic_label.shape)
    ...
    def collate_fn(self, batch):
        ...
        list_d_aug = []
        list_idx = []
        # augment batch (with Polarmix)
        scan_ids_pmx = []
        list_d_pmx = []
        list_d_pmx_aug = []
        list_idx_pmx = []

        oringinals = [item[0] for item in batch]
        augmenteds = [item[1] for item in batch]
        batch_id = 0
        for orig_data, aug_data in zip(oringinals, augmenteds):
            if orig_data is None:
                continue

            (scan_id, coord, feat, semantic_label, idx) = orig_data
        ...
        for orig_data, aug_data in zip(oringinals, augmenteds):
            ...
            (scan_id, coord_aug, feat_aug, semantic_label_aug, idx) = aug_data            
            scan_ids_pmx.append(scan_id)
            list_d_pmx.append((coord_aug[0], feat_aug[0], semantic_label_aug[0]))
            
            if self.use_sparse_aug:
                for aug_idx in range(1, self.positive_num+1):
                    list_d_pmx_aug.append((coord_aug[aug_idx], feat_aug[aug_idx], semantic_label_aug[aug_idx]))
            
            list_idx_pmx.append(idx.view(-1,1))

        # merge all the scenes in the batch
        coordinates_pmx_batch, features_pmx_batch, labels_pmx_batch = ME.utils.SparseCollation(dtype=torch.float32)(list_d_pmx)
        idx_pmx = torch.cat(list_idx_pmx, dim=0)
        
        batch_inds = coordinates_batch[:, 0]
        ...
        return_dict_pmx = {'scan_ids_pmx': scan_ids_pmx,
                       'batch_idxs_pmx': idx_pmx,
                       'coordinates_pmx': coordinates_pmx_batch,
                       'features_pmx': features_pmx_batch,
                       'labels_pmx': labels_pmx_batch,
                       'batch_size_pmx': batch_id,
                       'batch_inds_pmx': batch_inds
                       }

        if self.use_sparse_aug:
            coordinates_aug_batch, features_aug_batch, labels_aug_batch = ME.utils.SparseCollation(dtype=torch.float32)(list_d_aug)
            batch_inds_aug = coordinates_aug_batch[:,0]
            return_dict.update({'coordinates_aug': coordinates_aug_batch,
                                'features_aug': features_aug_batch,
                                'labels_aug': labels_aug_batch,
                                'batch_inds_aug': batch_inds_aug
                                })
        if self.use_sparse_aug:
            coordinates_pmx_aug_batch, features_pmx_aug_batch, labels_pmx_aug_batch = ME.utils.SparseCollation(dtype=torch.float32)(list_d_pmx_aug)
            batch_inds_pmx_aug = coordinates_aug_batch[:,0]
            return_dict_pmx.update({'coordinates_pmx_aug': coordinates_pmx_aug_batch,
                                'features_pmx_aug': features_pmx_aug_batch,
                                'labels_pmx_aug': labels_pmx_aug_batch,
                                'batch_inds_pmx_aug': batch_inds_pmx_aug
                                })

        return_dict_total = {'origin': return_dict,
                             'augment':return_dict_pmx}
    

        return return_dict_total
