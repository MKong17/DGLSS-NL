...
class ExpDGLSS(ExpSemantic):
    def __init__(self, model, cfg):
        ...
        self.warmup_epochs = cfg.npn.warm_up
        self.n_classes = cfg.model.out_classes
        self.config = cfg.npn
    ...
    def split_batch(self, batch):
        ...
        orig_batch = batch['origin']
        aug_batch = batch['augment']
            
        return orig_batch, aug_batch
    ...
    def forward(self, batch, use_metric=False):        
        ...
        if self.training:
            ...
            batch, aug_batch = self.split_batch(batch)

            coordinates = batch['coordinates'] # N, 3
            features = batch['features'] # N, 1

            coordinates_pmx = aug_batch['coordinates_pmx']
            features_pmx = aug_batch['features_pmx']            

            coordinates_aug = batch['coordinates_aug']
            features_aug = batch['features_aug']

            coordinates_pmx_aug = aug_batch['coordinates_pmx_aug']
            features_pmx_aug = aug_batch['features_pmx_aug']
            
            coordinates_aug[:,0] += batch['batch_size']
            coordinates_pmx_aug[:,0] += batch['batch_size']
            
            concat_coordinates = torch.cat((coordinates, coordinates_aug), dim=0)
            concat_features = torch.cat((features, features_aug), dim=0)
            sparse_tensor = ME.SparseTensor(coordinates=concat_coordinates.int(), features=concat_features) # spartensor 体素化
            predicted_sparse_tensor, encoder_tensor, encoder_tensor_bottle = self.model(sparse_tensor, use_both=True)
            encoder_coordinates=encoder_tensor_bottle.C

            concat_coordinates_pmx = torch.cat((coordinates_pmx, coordinates_pmx_aug), dim=0)
            concat_features_pmx = torch.cat((features_pmx, features_pmx_aug), dim=0)
            sparse_tensor_pmx = ME.SparseTensor(coordinates=concat_coordinates_pmx.int(), features=concat_features_pmx)
            predicted_sparse_tensor_pmx, encoder_tensor_pmx, encoder_tensor_bottle_pmx = self.model(sparse_tensor_pmx, use_both=True)
            encoder_coordinates_pmx=encoder_tensor_bottle_pmx.C
            
            if use_metric:
                encoder_tensor = self.metric_learner(encoder_tensor.F)
                encoder_tensor_pmx = self.metric_learner(encoder_tensor_pmx.F)
                
            return predicted_sparse_tensor, encoder_tensor, encoder_coordinates,predicted_sparse_tensor_pmx, encoder_tensor_pmx, encoder_coordinates_pmx, encoder_tensor_bottle 
    ...
    def training_step(self, batch, batch_idx):
        ...
        out_cat, bottle_feat, bottle_coor, out_cat_pmx, bottle_feat_pmx, bottle_coor_pmx, enc_bottle_cat = self.forward(batch, use_metric=use_metric)

        output_feat = out_cat.F
        output_feat_pmx = out_cat_pmx.F
        ouput_coor = out_cat.C
        output_coor_pmx = out_cat_pmx.C

        batch, batch_pmx = self.split_batch(batch)

        batch_size = batch['batch_size']
        labels = batch['labels']
        labels_aug = batch['labels_aug']

        labels_pmx = batch_pmx['labels_pmx']
        labels_pmx_aug = batch_pmx['labels_pmx_aug']

        loss = 0
        # segmentation loss
        labels_cat = torch.cat((labels, labels_aug),dim=0)
        labels_cat_pmx = torch.cat((labels_pmx, labels_pmx_aug),dim=0)

        # semantic_vox_loss =  self.criterion_sem(out_cat.F, labels_cat-1) #pre = out_cat.F, 
        semantic_vox_loss =  self.criterion_sem(output_feat, labels_cat-1) #pre = out_cat.F, 
        ...
        correlation_loss = losses.SCCloss(features=bottle_feat, 
                                          labels=labels_cat-1, 
        ...
        # NPN loss including Partial Label, Negative Lael and consistency regularization

        # coord, feat, labels = self.valid_fiter(labels_cat, output_feat, ouput_coor)
        coord_pmx, feat_pmx, labels_pmx = self.valid_fiter(labels_cat_pmx, output_feat_pmx, output_coor_pmx)
        # print(f"[labels check] shape of label {torch.unique(labels)} and label pmx {torch.unique(labels_pmx)}")

        npn_loss = losses.NPNloss(labels=labels_pmx-1,
                                  logits=feat_pmx,
                                  logits_b=bottle_feat,
                                  logits_b_pmx=bottle_feat_pmx,
                                  coords_b=bottle_coor,
                                  coords_b_pmx=bottle_coor_pmx,
                                  config = self.config)
        loss += npn_loss
        ...
        self.log('npn', npn_loss,          prog_bar=True, logger=False)
        self.training_step_outputs.append({
            'loss': loss.item(),
            'segmentation_loss': semantic_vox_loss.item(),
            'consistency_loss': consistency_loss.item(),
            'correlation_loss': correlation_loss.item(),
            'npn_loss': npn_loss.item()
        })        

        return {'loss': loss, 'segmentation_loss': semantic_vox_loss.detach(), 'consistency_loss': consistency_loss.detach(), 
                'correlation_loss': correlation_loss.detach(), 'npn_loss': npn_loss.detach()}
        
    def valid_fiter(self, labels, features, coordinates):
        valid_maks = labels != 0
        labels = labels[valid_maks]
        features = features[valid_maks]
        coordinates = coordinates[valid_maks]
    ...
    def on_train_epoch_end(self):
        ...
        self.log(f'{split}_npn_loss', npn_loss / len(training_step_outputs), rank_zero_only=True)
        return
